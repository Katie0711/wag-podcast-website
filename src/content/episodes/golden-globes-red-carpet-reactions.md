---
title: "We Rated the Golden Globes Red Carpet"
videoId: "z65YmriEg6Q"
publishDate: 2026-01-23
description: "The real episode YouTube calls \"We Tried Crashing the Grammys…\" is actually WAG rating the Golden Globes red carpet — the outfits the internet couldn't stop talking about, bold looks, confusing choices, and the back-and-forth debate over which stars nailed it."
heroImage: "https://img.youtube.com/vi/z65YmriEg6Q/maxresdefault.jpg"
topics: ["pop-culture-trends"]
---

The YouTube title says "We Tried Crashing the Grammys…" — but that's not actually what this episode is. What Angelina, Scarlett, and Annabella actually spent the episode doing is going through the Golden Globes red carpet, outfit by outfit, reacting to the looks everyone online was already arguing about.

## What the episode actually covers

It's a real-time reaction session: bold looks that landed, confusing choices nobody could agree on, and a few picks that had all three hosts going back and forth before settling on a verdict. Some outfits got an instant yes. Others split the room. That disagreement — not a script, not a pre-written "best and worst" list — is most of what makes the episode watchable.

## Why the real title doesn't match

WAG Podcast titles, like a lot of teen entertainment content, sometimes lean into a punchier hook than the literal episode content ("We Tried Crashing the Grammys…" gets a click "Slime recipe" never would). The real subject here is red carpet fashion commentary, not an actual attempt to crash an award show — worth knowing going in if you came looking for the title's premise specifically.

## More WAG Podcast

Looking for the rest of what WAG Podcast covers? Head to [Watch](/watch/) for recent full episodes, or [Segments](/segments/) for the recurring bits across the show.
