...
## Pattern: Named-person + specific-event

**Evidence base:** LOL Podcast (2026-09-10 competitive deep-dive) — 27 consecutive real titles, 27/27 following [Named Person] + [Specific Event], zero exceptions: "Harper Raged and Slapped Kate!", "The 67 Kid Asked Harper Out!", "Yolanda Stole Harper's Dog!", "Natalie Reynolds Is Pregnant!". Their own vidIQ outperformance data shows every recent overperformer (2.4x–6.7x channel average) follows this pattern and imports an outside personality; every underperformer (0.6x–1.0x) is internal-only. **WAG's own single relevant data point:** "Angelina CRASHED OUT for 35 Minutes Straight!" — WAG Podcast's best recent performer at 1.9x channel average, the only recent WAG Podcast title in this form. **This is n=1 for WAG — a real, suggestive result, not yet a validated pattern.** Log a second independent WAG instance (or the native A/B test in Test 2 below) before treating this as confirmed for WAG specifically, per this doc's own Rule-of-3 discipline used elsewhere.

1. **Why it works:** names a specific real person and a specific real event in the same breath — the viewer isn't asked to imagine a category of content, they're told exactly what happened to exactly who.
2. **Emotional trigger:** parasocial investment in the named person + curiosity about the specific event's resolution.
3. **Psychological mechanism:** specificity-as-credibility (same family as the hyper-specific-constraint pattern above) combined with parasocial narrative investment — a named person makes the claim feel like a real report, not a premise.
4. **Cold-audience friendliness:** low without prior cast recognition — this pattern is the opposite end of the spectrum from the hyper-specific-constraint/completeness-claim patterns above, which are high on cold-audience friendliness precisely because they don't require knowing anyone. Named-person titles work FOR an audience that already has some cast familiarity, and work BEST when the named person is a real outside guest with their own pre-existing audience, not an unfamiliar cast member.
5. **Dependence on existing fandom:** high for the named person specifically — LOL Podcast's strongest instances (Yolanda, Natalie Reynolds) import a person who already carries their own following into the title, rather than relying on the home audience already knowing an internal cast member.
6. **Relationship to the thumbnail:** thumbnail should show the named person's real, recognizable face/reaction — the title's specificity and the thumbnail's face should both point at the same real person, not split attention between a generic scene and a name in text.
7. **Relationship to the hook:** LOL Podcast escalates rather than resolves in the first 30-60 seconds — the title states the event happened, the hook makes the viewer wait to see it unfold.
8. **Browse vs. search intent:** primarily browse/suggested — nobody searches a specific named person's specific event cold; this is a recommendation-feed pattern, not a discovery-search pattern.
9. **Comment potential:** likely high (taking sides on a named person's real event), consistent with the relationship-ambiguity pattern's confirmed comment data above, but not independently measured for this exact pattern yet.
10. **When NOT to use it:** (a) for cold-discovery/growth content aimed at viewers with zero cast familiarity — this is the same caution already logged against the relationship-ambiguity pattern above, and applies even harder here since the entire mechanism depends on recognizing the name; (b) when the "named person" is a fabricated or exaggerated framing of a minor real moment — this pattern is exceptionally easy to overclaim with (declaring something "happened" that was actually minor or staged), which breaks WAG's content-authenticity rule directly; (c) don't force it onto WAG Podcast's internal-only cast when the real underlying event doesn't justify the drama-forward framing — LOL Podcast's own data shows internal-only conflict titles are their WEAKEST category (0.6x–1.0x), so this pattern's real lift appears to come specifically from importing outside people, not from dramatizing the existing cast harder.

**Standing comparison to run:** the next 6 WAG Podcast episodes should alternate named-person-event titles against WAG's current topic-based default (e.g. "Girls vs Boys Guess the Number Game!", "Is It Flirting if He DOES This?"), using a native YouTube title A/B test where possible, measuring CTR segmented by traffic source (not blended) plus 30-second retention — per WAG's own measurement-rigor standard, small-sample wins should not be read as confirmed. See `WAG_COLLABORATION_MAP.md` for how this pattern connects to bringing in real outside people the title can actually name.
