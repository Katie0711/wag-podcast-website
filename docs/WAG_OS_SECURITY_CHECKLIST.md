...
## 24. YouTube Reporting API (thumbnail impressions + CTR) — job created and proven live, 2026-08-11

**New tables, RLS-correct from creation:** `reporting_api_jobs` and `reporting_api_reports_ingested` (migration `youtube_reporting_api_job_tracking`) — both got `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` plus the standard `authenticated_full_access` policy in the same migration that created them, not as a follow-up fix. Applies the lesson from Section 23's `content_eras`/`video_cohorts` gap directly.

**New Edge Function:** `youtube-reporting-sync`, same hardened pattern as the other three YouTube functions (`withSupabase({auth:'secret:wag_hq_backend'})`, connector lookup, refresh-token-via-Vault, `ALLOWED_SCOPES` assertion on every token refresh — no scope outside `youtube.readonly`/`yt-analytics.readonly` is accepted). Two explicit, separately-called modes (`create_job`, `sync_reports`) — no scheduling, per Katie's explicit instruction to prove the workflow manually first.

**Real scope question, resolved live, not assumed:** the official `jobs.create` reference doc's authorization note names `yt-analytics-monetary.readonly`, which WAG's connector does not have. A live test resolved this cleanly: the first attempt returned `403 PERMISSION_DENIED` with the message *"YouTube Reporting API has not been used in project 893203037851 before or it is disabled"* — a project-level API-enablement gate, unrelated to OAuth scope. Katie enabled the API in Google Cloud Console (one-time admin action, no new consent or scope). Re-running the identical live test immediately after returned `200 OK` with a real job (`google_job_id: b4c38e98-744c-4a68-aa1d-22ce7f694b2a`, `report_type_id: channel_reach_basic_a1`, connector 1 / WAG Main) — confirming the existing `yt-analytics.readonly` authorization is genuinely sufficient for this non-monetary report type. No scope expansion needed or requested.

**Status:** job created and persisted (`reporting_api_jobs` row 1). First report generation is on Google's own schedule, typically ~24-48h after job creation — `sync_reports` has not yet been run against real data since nothing exists to pull yet. No polling loop or cron was introduced; the next `sync_reports` call will be a deliberate, manual, one-time check once Katie confirms enough time has passed, exactly as instructed. When it runs, it will ingest `video_thumbnail_impressions`/`video_thumbnail_impressions_ctr` into `yt_raw_observations` under `metric_source='youtube_reporting_api'`, log the report in `reporting_api_reports_ingested` to guard against Google's limited retention window, and feed back into a Packaging Intelligence/Greenlight v3 re-run on the same garage-idea test case.

## 26. Gmail Revenue & Partnerships connectors — both live, 2026-08-11

**Two independent, WAG-owned connectors**, per Katie's explicit directive to use the same ownership pattern as YouTube (WAG's own Google Cloud OAuth app, not Claude's own ad-hoc session connector). New `gmail_accounts` table (RLS enabled + `authenticated_full_access` policy from creation, plus an immutable-binding trigger mirroring `yt_channels`'), three new Edge Functions (`gmail-oauth-start`, `gmail-oauth-callback`, `gmail-disconnect`) following the exact YouTube connector security sequence: nonce-based state (replay/CSRF-proof), `ALLOWED_SCOPES` assertion rejecting any grant beyond `gmail.readonly`, and a permanent Gmail-address binding per connector that refuses a later authorization resolving to a different address.

**Scope, verified against Google's own docs before building, not assumed:** `https://www.googleapis.com/auth/gmail.readonly` only, for both accounts. Checked specifically whether any Gmail scope grants draft-creation without also granting send — none does (`gmail.compose` bundles both) — so this connector's design deliberately keeps "draft" to mean a recommended response stored in WAG Brain for Katie to send herself, never an actual Gmail draft object, and never requests `gmail.compose`.

**Two real, found-and-fixed blockers during first authorization, both diagnosed live rather than guessed at:**
1. **`redirect_uri_mismatch`** — the existing Google Cloud OAuth client had never been told about the new `gmail-oauth-callback` URL. Fixed by Katie adding it to the client's Authorized redirect URIs (one-time Console step).
2. **`access_denied` / "has not completed the Google verification process"** — confirmed the OAuth app is genuinely in Testing publishing status (not Production, contrary to an earlier assumption based on the YouTube connector's multi-week uptime). Fixed by Katie adding both `partners@wagstudios.co` and `thewildadventuregirls@gmail.com` as named Test Users in the OAuth consent screen — the same reason the YouTube connector has worked without incident (its account was already on that list).
3. **Gmail API itself not enabled for the project** — same class of gate as the YouTube Reporting API earlier this session, surfaced via a diagnostic improvement to `gmail-oauth-callback` (mirrors the Reporting API's sanitized-error-detail pattern) rather than guessed at. Fixed by Katie enabling the Gmail API in Cloud Console.

**Real, unresolved open question, flagged rather than assumed away:** Testing-status OAuth apps get refresh tokens that expire after 7 days for non-basic scopes. The YouTube connector hasn't visibly hit this in weeks of real use — whether that's because Google resets the expiry clock on regular refresh-token use, or some other reason, is not confirmed. This needs real monitoring once the Gmail connectors have been live past 7 days, not an assumption that the same silence will hold.

**Verified live, both accounts, confirmed via direct DB query not just the consent-screen UI:**
- `connectors` id=4 (`partners@wagstudios.co`) — `status=active`, `revocation_status=active`, credential in Vault.
- `connectors` id=5 (`thewildadventuregirls@gmail.com`) — same.
- `gmail_accounts` — one binding row per connector, real resolved addresses matching expected.
- `activity_log` — real success entries for both, `scope=https://www.googleapis.com/auth/gmail.readonly` confirmed for both, no broader grant accepted.

Re-ran `get_advisors` after all schema/credential changes — clean except the same 3 pre-existing INFO items and the accepted `pg_net` WARN, no new findings.

**Not yet built:** any actual email read/search/classify logic (per `WAG_REVENUE_EMAIL_INTAKE_ARCHITECTURE.md`'s design — purpose-limiting the legacy inbox via narrowed search queries and a relevance-classification discard gate, anti-fraud verification states, prompt-injection containment). Connection and authorization is proven; the bounded historical recovery pass is the next real step.
