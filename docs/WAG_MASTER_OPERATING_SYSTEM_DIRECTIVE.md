...
## 62. Opportunity Quality Correction + VP-of-Partnerships Intelligence Upgrade + Company-Wide Intelligence & Performance Standard (recorded 2026-08-12, mapped, not built)

Four related messages, received in sequence while the Revenue pause-work was in progress. Per Katie's explicit instruction each time ("do not derail," "do not build this now," "record so future implementations conform"), this is a mapping into the existing §59-61 structure, not new architecture. Full text preserved in session history.

**1. Opportunity Quality Correction (real, immediate, already applied):** GoPro's Grom Quest was surfaced as WAG's top Outbound Opportunity Intelligence proof, but it's a contest/creator-award program, not a paid partnership. Corrected live in `WAG_REVENUE_RECOVERY_PASS_FINDINGS.md` §15 and `WAG_REVENUE_DECISION_INTELLIGENCE.md` §0. **Standing rule now in force:** contests, sweepstakes, affiliate-only programs, and free-product/gifting programs do not compete with real paid-partnership prospects in Katie's executive queue — they may be logged separately if genuinely valuable. Applied and already caught real cases in the very next research round (Chomps, Scrub Daddy, Gabb Wireless — held at "Prospect" not "Qualified Prospect" specifically because of this rule, per Decision Intelligence §2).

**2. Leads → Opportunities ladder + Commercial Opportunity Quality test — now standing, in use:** `Market signal → Prospect → Qualified prospect → Opportunity → Active deal → Partner`, and a 10-point evaluation (commercial evidence, audience fit, brand safety, creative fit, property fit, timing, budget probability, access path, strategic upside, rights/risk) with an explicit no-manufactured-scores rule (UNKNOWN stays UNKNOWN). Both now recorded in `WAG_REVENUE_DECISION_INTELLIGENCE.md` §0 and applied to every ranked prospect there — this is real, in-use discipline, not a future plan.

**3. VP-of-Partnerships intelligence upgrade — mapped against existing infrastructure, mostly not net-new:**
- Money-in-motion / category intelligence, Agency Intelligence as a first-class function, WAG Commercial Fit Model, creative-commercial-intelligence (does the sponsorship become good content) — genuinely new emphases; Agency Intelligence research now actively running (background agents, `WAG_REVENUE_DECISION_INTELLIGENCE.md` §8 once synthesized).
- Relationship memory (people, not just companies), temporal intelligence (source/observed-date/confidence/freshness), negative-evidence learning (ghosts/rejections/counteroffers as data), pricing-learns-from-negotiation — all already the *substance* of §60's memory-type taxonomy and §33's agent/manager evaluation; this message sharpens Revenue's specific application of it, doesn't add a new system.
- Manager-challenges-specialist discipline, outcome-based (not activity-based) department scoring — already §61's binding constraint (don't score on activity, don't score without real evidence).

**4. Company-wide Intelligence & Performance Standard — every department inherits this, mapped against §59-61, genuinely new pieces flagged:**
- Specialists → Managers → Department Leaders → Chief of Staff → Katie hierarchy — extends §59's Wave sequencing with an explicit escalation chain; Chief of Staff role is new, not yet named anywhere else in WAG OS.
- Learning loop as *separate, un-collapsed* stages (Observation → Hypothesis → Prediction → Recommendation → Decision → Action → Outcome → Postmortem → Lesson → Updated behavior) — already structurally true of the `predictions`/`outcomes` table split (§60), this message makes the full chain explicit and names Postmortem as a distinct stage, which is new.
- Score predictions, not confidence-sounding language; component scores over magic numbers; time windows/trends; preserved disagreement (don't force consensus) — all extend §61's decomposable-scores/anti-gaming discipline; "preserve disagreement" is a genuinely new, explicit rule not previously stated.
- Katie's executive scorecard + a separate CEO decision-performance view (not a personal grade, tracks recommendation-vs-decision-vs-outcome to learn from both Katie's overrides and the system's misses) — restates §61's already-explicit "not a personal grade" framing; the decision-performance tracking mechanic (was Katie right, was the agent right, learn from both) is a sharper, genuinely new mechanic worth naming.
- Brain-must-remember-without-becoming-a-junk-drawer, knowledge decay/expiration, historical facts stay historical (never silently overwritten) — directly extends §58's absence-of-evidence standard into a symmetric presence-of-evidence-decays standard; genuinely new half of the same principle.
- Shared external intelligence (don't have 30 agents re-researching the same thing) — the exact discipline already applied ad hoc in `WAG_REVENUE_DECISION_INTELLIGENCE.md` §7; this message makes it a company-wide requirement, not just a Revenue habit.
- Capability-gap self-identification, cross-department knowledge routing (named examples: Revenue↔Main/Podcast, Licensing↔Publishing) — both genuinely new, not previously named.
- No autonomous executive authority — restates the existing Katie-in-Control model (§37, §34) verbatim; no change.

**5. Implementation-detail concepts to apply when eventually built (explicitly deferred, recorded so future work conforms):** per-agent Agent Charter (job/manager/inputs/tools/outputs/decision authority/escalation triggers/KPIs/evidence requirements/known gaps/version), per-manager Manager Charter (quality-control duties, corrective-action loop, self-review), a common evidence hierarchy (`FIRST-PARTY WAG DATA / PRIMARY EXTERNAL SOURCE / VERIFIED SECONDARY SOURCE / INDUSTRY BENCHMARK / OBSERVED PATTERN / HYPOTHESIS / UNKNOWN` — sharper than the existing `revenue_pricing_evidence` tier enum, should eventually supersede/align with it), confidence tracked separately from recommendation, escalation-severity levels (`INFORMATION / WATCH / ACTION NEEDED / KATIE DECISION / URGENT-RISK`, with the standing exception that every legitimate inbound opportunity still reaches Katie regardless of severity), version history on any agent method change, an eventual override-calibration system (preserve original recommendation + Katie's decision + reason + outcome, to learn from both directions), and need-to-know access boundaries even inside WAG Brain (agents don't auto-consume everything).

**Binding constraint, stated four times across these messages and carried forward here:** none of §62 gets built now. It governs how future department/performance-management work gets built, once §61's own binding constraint (real evidence must exist first) is satisfied. The real work this pass — Revenue ranking, pricing framework, licensing strategy, Agency/AI/Gaming category research — continues under the corrected standard in real time; that is the actual deliverable, not this recording.

Returning to the Revenue build.

## 62b. Revenue transitions from setup project to ongoing function (recorded 2026-08-13)

Per Katie's explicit instruction: "Revenue should become an ongoing department, not an endless setup project... do not let Revenue consume the entire WAG OS build." After this pass (10 executive cards, a real agency network with named contacts, a TIMING dimension, a 5-state WATCHLIST, and package-opportunity evaluation — all in `WAG_REVENUE_DECISION_INTELLIGENCE.md`), Revenue Opportunity/Agency/Pricing/Licensing Intelligence continues as a standing, lower-cadence function (new discovery rounds, watchlist rechecks on their real recheck dates, card-building as evidence justifies it) rather than the primary focus of every session. WAG OS attention returns to the older backlog (wagmediapartners.com decision, age-data surfacing, WAG Media Playbook, Phase 2A implementation, WAG OS certification runs, MFA) and begins real work on §63 (YouTube Growth & Audience Obsession) next — both explicitly named by Katie as at least as important as Revenue monetization, not secondary to it.

## 63. Company-Wide YouTube Growth & Audience Obsession Directive (recorded 2026-08-12, mapped, not built)

Applies to WAG Main and WAG Podcast — distinct properties, shared external intelligence where useful, per the standing property-separation rule (§22a/§22b, carried through Revenue's own property rules in §§37-38). Full 23-point text preserved in session history. **Mapped against `WAG_MAIN_INTELLIGENCE_LOOP_V1.md` (Retention/Packaging/Format Intelligence + Greenlight Manager v3, already real and proven on one real video) and `WAG_PODCAST_DEPARTMENT_ARCHITECTURE.md` (GM + 19 specialist functions, design-only). Not built now — recorded so it governs the next real extension of either loop.**

**Already substantially covered, not net-new:**
- Format Scout's MUST FILM/HIGH-PRIORITY TEST/WATCH/NOT FOR WAG bands — same shape as Greenlight Manager v3's existing scoring discipline (explicit no-fake-certainty rule already standing, §wag_video_greenlight_system_directive equivalent) and Main Intelligence Loop's Format Intelligence specialist.
- Retention/Story Engineering using historical evidence before filming, not just post-mortem — already the proven v1.1 retention-to-content-alignment work (real chapter markers, n=2 videos, explicitly flagged as needing more comparable videos before being trusted beyond directional).
- Packaging Intelligence (title+thumbnail together, CTR/traffic-source-aware) — already a named Main Intelligence Loop specialist; this directive adds that the Reporting API's populating reports will materially strengthen it once live, which is already the known, tracked next step (§395/§396 build history).
- Greenlight as a pre-production quality gate synthesizing multiple specialists without a "40-agent meeting" — already Greenlight Manager v3's exact design intent.
- Podcast-specific intelligence (topics, guest chemistry, clipability, packaging) — already named in the 19-function Podcast Department Architecture, not yet built.
- Active experimentation with a hypothesis→test→outcome→lesson record, no multi-variable confounding — already the Experiment Registry's standing discipline (`WAG_EXPERIMENT_REGISTRY.md`).
- Property-aware routing (MAIN/PODCAST/BOTH/NEITHER) for any shared signal — same pattern already proven in Revenue's own cross-property routing (§22-24 equivalent, Consolidated Architecture).
- Performance Management inheritance (Must-Film win rate, title/hook prediction accuracy, trend-alert lead time) — already §61/§62's prediction-scoring discipline, applied here rather than newly invented.
- Benchmarks from WAG's own real historical data, not invented universal numbers, separated by property — already the standing rule throughout WAG OS (e.g. Main's real 23,991-avg/Podcast's real 11,124-avg views kept structurally distinct, never blended).

**Genuinely new specialist functions, not currently named anywhere — recorded for the next real build, not built now:**
- **YouTube Market Intelligence/Scouting** and a living **Competitive Creator Watchlist** that evolves (creators drop off, not permanent) — broader than the existing Format Intelligence specialist, which reasons from WAG's own data more than external creator scouting.
- **Title Intelligence** as its own specialist (MUST TITLE/TITLE OPPORTUNITY tracking, title-mechanism-not-copying discipline) — currently folded into Packaging Intelligence; this directive splits it out.
- **Hook Lab** — first-seconds/first-minute treated as its own specialist domain, predictions checked against real retention curves; not currently separated from general Retention Intelligence.
- **Personality & Cast Intelligence** — a genuinely new function: understanding *why* viewers love the specific cast dynamic, with an explicit rule that editing/optimization must preserve personality moments even at some retention-efficiency cost. This is a real, named tension with pure retention optimization, worth keeping distinct rather than folding into Retention Intelligence, which could otherwise optimize personality away.
- **Audience Obsession/Fandom** and **Returning-Audience Intelligence** — distinct from view-count optimization; asks what creates anticipation/habit/return-viewing, not just first-view performance. Explicitly must not invent new-vs-returning-viewer data the Reporting API doesn't actually provide (per the standing absence-of-evidence rule, §58).
- **Shorts Intelligence as its own specialist**, not a leftover of long-form — already named once as a build item (Task #323, completed) but this directive asks for continuous external Shorts-trend monitoring, not just WAG's own Shorts performance.
- **Trend & Cultural Radar** — explicit trend-must-pass-5-questions filter (relevant/timely/brand-safe/WAG-can-improve-it/audience-will-care) before any trend becomes a WAG idea. New, not currently a named function.
- **Format IP tracking** — treating WAG's own repeatable formats as a tracked asset (name/premise/rules/performance history/fatigue), the long-term alternative to permanent trend-chasing. New.
- **External Breakout Alerts** with named alert types (MUST WATCH/MUST TITLE/MUST FILM/SHORTS ALERT), evidence-gated so Katie isn't overwhelmed with ordinary trend noise. New, extends §61's escalation-severity concept (§62) into a YouTube-specific alert taxonomy.

**The one framing worth preserving verbatim, not just mapping:** Katie's own distinction between a **discovery engine** ("what will make people want to click this") and an **obsession engine** ("why will they come back next Sunday") — trend intelligence serves the first, fandom/personality/cast intelligence serves the second, and building only one produces either a channel that chases whatever's hot with no loyalty, or a channel with a loyal small audience that stops growing. Both are required, and the obsession engine is explicitly what protects the girls' real personality/humanity from being edited away in service of retention metrics alone — the same concern already implicit in the Retention Intelligence work's own honest v1.1 caveats.

**Binding constraint, same as §62:** not built now. Governs the next real extension of the Main Intelligence Loop or the Podcast Department build, whichever comes first per the Wave sequencing (§59-60) — not a new department, not a rebuild of what's proven.

Returning to the Revenue build.

## 64. EXISTENCE ≠ OPERATIONAL — permanent maturity-state standard (recorded 2026-08-13)

Established after a real, evidence-based YouTube Growth operational audit found multiple cases of a capability existing structurally while never actually running: a Reporting API job with a real Google job ID and zero ingested reports; `yt_channel_baselines` fully specified in schema with no function anywhere that computes it; `yt_raw_observations` (1,609+ rows) that is a single one-time backfill from one 63-minute window, not continuous capture; 8 real predictions with 8 outcome rows that are all `status='pending'` — a prediction pipeline that has never once closed the loop against reality; real retention curves captured and stored, read by nothing. **This is now a permanent, company-wide principle, not a Revenue or YouTube-specific rule.**

**The rule:** a capability may not be described as operational, live, working, automated, proven, or complete merely because any of the following exists — a database table, schema, function, Edge Function, cron definition, API job, connector, agent, employee record, skill, architecture document, prompt, or prediction. Existence is not operation.

**Maturity states, used wherever WAG OS reports capability status:**
`DESIGNED` (spec/doc exists) → `BUILT` (executable implementation exists) → `CONNECTED` (real data/service connections work) → `TESTED` (successfully run against a controlled or real test) → `OPERATING` (runs in its intended production workflow) → `VERIFIED` (outputs independently checked against expected real behavior) → `LEARNING` (outcomes feed back into future behavior, with real evidence the feedback is actually used).

**Concrete calibration, from the real cases that prompted this:** the Reporting API job is `BUILT`+`CONNECTED` (a real job exists, a real Google job ID was issued) but not `TESTED` or `OPERATING` (zero reports ever ingested). `yt_channel_baselines` is `DESIGNED` only — schema exists, zero implementation. `yt_raw_observations` capture is `TESTED` (one real successful run, four sync-log entries) but not `OPERATING` (no recurring schedule). The freshness-reviewer cron is the one real `OPERATING` example found in this audit — real, dated, recurring executions with real results, proof the pattern works when actually wired up. The Greenlight/prediction pipeline is `TESTED` (real predictions generated against real evidence) but not `LEARNING` — no outcome has ever closed, so no lesson has ever fed back into a later recommendation. Podcast's 19 documented specialist functions are `DESIGNED` only, 0% further.

**Explicit anti-patterns, named because they already almost happened this session:** a cron definition that exists but has never fired is not "operating." An agent that produced one recommendation is not automatically "learning." A table with real columns and zero populated rows is not a working capability. Do not let a future report round a `DESIGNED` or `BUILT` capability up to "operational" because the schema looks complete — the schema looking complete is exactly the failure mode this rule exists to catch.

**Ties into performance management (§61-62):** once department/manager scoring is real, a manager reporting a `DESIGNED` or `BUILT` capability as `OPERATING` or `LEARNING` should be scored down for the misrepresentation itself, not just for the underlying capability gap — an honest "this is BUILT, not yet OPERATING" report is worth more than an inflated "it's working."

**Org-model inconsistency, recorded as a finding, not fixed:** the same audit found Revenue has real, substantial operational infrastructure (19 tables, real deal/approval/evidence data, a working catch-up ingestion) with **zero representation** in the `employees`/`departments`/`roles` model — no Revenue employee row exists anywhere. Podcast has the opposite problem: real department-architecture documentation with **zero operational capability and zero employees**. Per Katie's explicit instruction, no employee rows are being created for either right now merely to make the org chart look complete. The real requirement, for whenever the company/department performance layer gets built: a clean distinction between **Capability, Agent/Employee, Manager, Department, Workflow, and Outcome** — not every real capability needs a fake employee wrapper, and not every documented department has earned one yet.

### 64a. Platform-change claims require primary-source verification when a primary source should exist (recorded 2026-08-12)

Established after Creator & Platform Intelligence Proof 1 found a discovered "YouTube Shorts algorithm change" signal — specific, confidently phrased, repeated across a cluster of SEO content-farm sites — was substantially fabricated: three of four claimed mechanisms had zero attribution to YouTube's own blog, documentation, or Creator Insider communications, and a fourth was a misdated 2023 policy recycled as fresh 2026 news. **This is now a permanent, company-wide research principle, not a Creator Intelligence-specific rule** — the same failure mode can hit Revenue's platform research, SEO/AEO work, or any department reading trade coverage of a platform change.

**The rule:** a third-party article claiming a platform (YouTube, Google, Spotify, Apple, or any other) changed its algorithm, policy, or product cannot become actionable WAG strategy merely through repetition across secondary sites, however fluent or confident the phrasing. Before any platform-change claim informs a WAG decision, search for the underlying primary evidence — the platform's own blog, official documentation, policy page, Creator Insider communication, or equivalent — and cite it directly. If no primary source can be found, classify the claim **UNVERIFIED** explicitly, not confirmed-by-volume.

**Not a mechanical hierarchy everywhere.** This does not apply where primary evidence legitimately cannot exist (e.g. a creator's private motivations, an unannounced internal decision). Platform-announcement claims specifically are a case where a primary source generally *should* exist — its absence is itself informative, not just an evidence gap to shrug past.

Recorded permanently in WAG Brain as `canonical_facts.platform_change_claims_require_primary_source_verification`.

### 64b. Intelligence must drive outcomes — every department owns a business result, not a research volume (recorded 2026-08-12)

**No WAG department exists primarily to generate research, reports, articles, dashboards, alerts, or recommendations.** Research/information is an input into an outcome; it is never the outcome itself. Every operating department must own an explicit company result — e.g. Creator & Platform Intelligence's mission is not "monitor creators," it is *identify meaningful external changes early enough, understand their mechanisms accurately enough, and translate them into actions that materially improve WAG Main and WAG Podcast.* Main's mission is not "run Greenlight," it is *systematically increase breakout reach, CTR, retention, returning viewership, fandom, and repeat-episode anticipation.*

**Bad goals, explicitly named so they're never scored as good ones:** "research 100 creators," "publish 30 articles," "generate 50 leads," "produce 100 recommendations." These are operational inputs. A department hitting all of them while moving no real outcome has not succeeded.

**The goal/execution template** every operating department should eventually carry: `MISSION → NORTH STAR OUTCOME → CURRENT BASELINE → 90-DAY/QUARTERLY GOAL → TARGET → STRATEGIES → ACTIVE EXPERIMENTS → OWNER/MANAGER → PROGRESS → ON TRACK/AT RISK/BEHIND → DIAGNOSIS → CORRECTIVE ACTION → FINAL RESULT → LESSONS → NEXT GOAL.` Confirmed (2026-08-12 audit) that **no clean version of this template exists anywhere in WAG OS today** — recorded here as a shared company capability to build once and apply everywhere, not as a department-specific system, and not built yet.

**The execution-funnel this ties to:** `DETECT → VERIFY → SCORE → RECOMMEND → ACTION PLAN → MANAGER REVIEW → KATIE DECISION (where required) → ASSIGN → EXECUTE → MEASURE → LEARN`. A high-scoring recommendation needs a clear owner, deadline/opportunity window, expected result, measurement plan, status, and an escalation rule if it stalls. **WAG Brain must not become a graveyard of excellent recommendations nobody executes** — this is a real, already-observed risk (real flagged Revenue leads currently sitting with no owner or deadline is the concrete example), not a hypothetical one.

Full design detail: `WAG_INTELLIGENCE_OUTCOMES_AND_ORG_GAPS.md`.
