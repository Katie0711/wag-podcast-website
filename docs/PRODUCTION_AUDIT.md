...
- **Real gap found and fixed:** the `Verdict Reveal Recipients` segment (drives the reveal automation) was stale at `num_members: 0` and hadn't recalculated since the account was set up. Triggered a manual recalculation via the Beehiiv API — segment is now current. This means when the branch does go live, the automation will actually pick up real qualifying subscribers instead of silently missing them; this class of staleness is worth a recheck right before launch day, not just once now.
