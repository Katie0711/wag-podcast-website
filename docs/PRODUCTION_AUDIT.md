...
**Evidence:** FAQPage schema present and content-matched (not duplicated between visible copy and schema) on Growing Up, Segments, FAQ, Guys Answer Questions, Who's Most Likely To, Teen Comedy Podcast, Video Podcasts for Teens, and all episode pages with real FAQs — verified via direct JSON-LD parse this session on thewagpodcast.com's newly-fixed pages. **Correction:** this line previously claimed Growing Up and Segments already had FAQPage schema when in fact they only shipped `CollectionPage`/`ItemList` — confirmed false via direct grep. Both were fixed this pass (real FAQ content added, driving matching `FAQPage` schema); Teen Comedy Podcast and Video Podcasts for Teens were also fixed the same pass (previously `WebPage`-only despite already having direct-answer-shaped body copy).
...
**Production capability check, per Katie's explicit request (2026-08-07) — real findings, not just "it's configured":**
- **`BEEHIIV_API_KEY` in true production (`main`):** *as of this specific check, pre-merge* — cannot be verified as "working in production" because there is nothing to test — `git ls-tree origin/main` confirms **zero files under `src/pages/api/`** on production `main` right now. The entire interaction platform, including WAG Verdict, exists only on the unmerged `feature/wag-match-quiz` branch (confirmed `feature/wag-verdict-mvp` is an ancestor of it — one linear branch, not two). **Correction to this doc's own earlier framing:** Verdict was previously described elsewhere (`ARCHITECTURE.md`, `ROADMAP.md`) as "already production-verified" — that was imprecise and has been corrected in both files. It was verified pre-launch (dev/staging), never deployed to `main`. Nothing in the interaction platform has a live production API endpoint today.
- **RESOLVED 2026-08-15 (stale-doc correction, not a new event):** the merge described as pending above actually happened the same day this check was written — `feature/wag-match-quiz` → `main`, commit `3bcfdb1`, per `MASTER_STATUS.md` and `CHANGELOG.md`. Re-verified directly against real git state this pass: `git ls-tree -r origin/main --name-only` confirms `src/pages/api/interaction-consent.ts`, `poll-vote.ts`, `verdict-vote.ts`, and `src/components/PollWidget.astro` / `QuizWidget.astro` / `VoteWidget.astro` are all present on `origin/main` right now — the interaction platform **is** deployed to production. This section was never updated after the merge landed, so it sat here reading as current, live doubt about something already resolved. Leaving the pre-merge findings above intact for the historical record (real diagnostic value — showed the checklist worked), but they describe a snapshot in time, not the current state. Current state matches `MASTER_STATUS.md`'s "🟢 LIVE in production" line — no real interaction-platform-deployment contradiction remains across these 4 docs as of this correction.
- **API calls succeed / tags apply:** confirmed on the Beehiiv side — all 6 interaction tags exist, `verdict` tag shows `subscriber_count: 3` in Beehiiv's own tag list. But cross-checked against every real subscriber in the publication (5 total, all statuses: active/pending/inactive/needs_attention) and **none currently carry the `verdict` tag** — the 3 are almost certainly leftover from earlier dev/staging testing, not real users, and the tag's cached count is stale relative to current membership. Not a bug to fix, just an accurate read of what the number means.
- **Automations fire:** both live automations (`Verdict Reveal Notification`, `WAG Insider Welcome`) show `enrolled_count: 0` — expected, since no real production traffic has ever hit these endpoints.
- **Real gap found and fixed:** the `Verdict Reveal Recipients` segment (drives the reveal automation) was stale at `num_members: 0` and hadn't recalculated since the account was set up (`last_processed_at` was a day old). Triggered a manual recalculation via the Beehiiv API — segment is now current. This means when the branch does go live, the automation will actually pick up real qualifying subscribers instead of silently missing them; this class of staleness is worth a recheck right before launch day, not just once now.
- **Bottom line:** the Beehiiv-side infrastructure (tags, segments, automations, whitelist) is real, correctly built, and ready. What can't be confirmed until the branch is actually merged is whether Netlify's **production** environment (not preview/dev) has `BEEHIIV_API_KEY` set — that's a Netlify dashboard check only Katie can do directly (no CLI/dashboard access available from this environment), and it should happen as part of the actual merge/launch step, not before.
...
- [ ] Before calling any "email me X" flow production-verified, confirm the code path is actually deployed to `main` — dev/staging verification and production verification are not the same claim, and docs should say which one happened
- [ ] Right before launch (not weeks before): recalculate every dynamic segment tied to a live automation and confirm `num_members` matches current tag counts — dynamic segments can go stale between recalculations
