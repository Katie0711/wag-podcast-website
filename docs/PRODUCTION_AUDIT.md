...
- **`BEEHIIV_API_KEY` in true production (`main`):** cannot be verified as "working in production" because there is nothing to test — `git ls-tree origin/main` confirms **zero files under `src/pages/api/`** on production `main` right now. The entire interaction platform, including WAG Verdict, exists only on the unmerged `feature/wag-match-quiz` branch (confirmed `feature/wag-verdict-mvp` is an ancestor of it — one linear branch, not two). **Correction to this doc's own earlier framing:** Verdict was previously described elsewhere (`ARCHITECTURE.md`, `ROADMAP.md`) as "already production-verified" — that was imprecise and has been corrected in both files. It was verified pre-launch (dev/staging), never deployed to `main`. Nothing in the interaction platform has a live production API endpoint today.
