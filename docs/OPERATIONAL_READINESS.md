...
**Fixed 2026-08-08:** added a small, subtle "By submitting, you agree to our Privacy Policy" line directly on `ConsentCheckboxPair` (every interaction that captures email reuses this one component, so this covers all of them at once), linking to the canonical `wildadventuregirls.com/privacy/`. Styled deliberately quiet — muted color, small type, no age gate — visibility without adding friction. Verified rendering in the built page output.
