...
| `WAG_VIDEO_GREENLIGHT.md` | The per-idea record data model (idea → greenlight → package → produce → publish → measure → learn) and the shared 7-criteria scorer, honestly bounded against fake-certainty scoring; now a real, populated log (Round 1, 10 scored ideas) | Future real episode planning; logged as a Software Incubator opportunity in `SOFTWARE_INCUBATOR.md` |
