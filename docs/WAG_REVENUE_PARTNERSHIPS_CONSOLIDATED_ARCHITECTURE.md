...
## 38. Revenue Operating Layer — smallest activation proposal (2026-08-11, for Katie's review)

**The question this section answers, exactly as Katie posed it: which first specialist functions, doing what, with what evidence/tools, prohibited from what, surfacing to Katie how.** Not the full 12-15-role department (Department Architecture §4) — the smallest real team that can run both required loops: `detect legitimate inbound → verify → research → surface to Katie`, and `research prospective brands → score fit → develop opportunity/package → recommend outreach to Katie`. Nothing in this section is active until Katie approves it; it is a proposal, matching the same review-before-build discipline as the schema itself (Section 13, step 4, which required approval before the migration ran).

**Reuse principle, stated first because it governs everything below:** Inbound and Outbound Opportunity Intelligence are two entry points into the *same* shared intelligence, not two parallel systems. Both call the same Brand-Fit check, the same Pricing Intelligence (per the new Methodology doc), and read/write the same Relationship Brain (Section 25) and evidence tables (`WAG_REVENUE_RECOVERY_PASS_FINDINGS.md` §9). No function below owns a private copy of company, pricing, or relationship data.

### 38a. Inbound Opportunity Intelligence

**What it does:** monitors real, already-connected sources (the two Gmail connectors, bounded per Gate 1/Gate 2, `WAG_REVENUE_EMAIL_INTAKE_ARCHITECTURE.md`) for legitimate commercial inbound, verifies the sender/company is real, does basic company research (what do they actually make, is the product age-appropriate), and produces the executive opportunity card (Section 34c) — proven twice already on real cases (Pop&Boom retrospectively, Aha World prospectively, Recovery Pass Findings §11/§14b).

**Evidence/tools:** `gmail-recovery-search` (bounded query, never a full mailbox list); `revenue_opportunity_verification`'s five-tier classification; WebSearch for real company/product verification; `revenue_companies`/`revenue_deal_parties` to record the real counterparty and, where an intermediary is involved, the real underlying brand (Section 33a).

**Prohibited:** any reply or contact of any kind; any pricing statement to the counterparty; creating a deal past `needs_katie_review` without a resolved `revenue_approvals` row; suppressing anything short of confidently-classified `SPAM` (Section 34b) — a `NEEDS_VERIFICATION` case like Aha World is surfaced, never quietly held back pending more certainty.

**Reaches Needs Katie via:** a `revenue_approvals` row (`action_type=pursue_opportunity`, `status=needs_katie`) plus the executive opportunity card, exactly as built for Aha World (Recovery Pass Findings §14b).

### 38b. Outbound Opportunity Intelligence (Sponsor Scout)

**What it does:** researches real, currently-operating brands that could legitimately sponsor/license/partner with WAG, scores real fit (product/service appropriateness, target-demographic overlap, brand-safety per the Pop&Boom lesson), determines WAG Main vs. WAG Podcast vs. cross-brand fit (Section 24's routing), develops a real, specific partnership concept (not "sponsored video" — an actual integration idea grounded in WAG's real content buckets), and produces a preliminary commercial-potential estimate via Pricing Intelligence (never a committed number). First real proof run same-day as this proposal: 15-20 real brand candidates researched and ranked (below).

**Evidence/tools:** WebSearch for real, current company/marketing-activity verification (never invented companies or facts); WAG's own real performance data (`yt_raw_observations`) to ground property fit and rough tier estimation; the Pricing Intelligence Methodology (source-quality hierarchy, §2) for any external rate context cited; `revenue_companies` to check whether a candidate already has a real relationship on file (never re-pitching a declined or already-engaged company blind).

**Prohibited:** any outreach, any contact, any pricing commitment, any implied offer — this function produces a ranked research brief for Katie's decision, never a sent message. Never recommends a brand that fails the real Pop&Boom-derived brand-safety screen (age-restricted products, dating apps, alcohol/gambling/vaping, anything requiring 18+ users or on-camera talent).

**Reaches Needs Katie via:** a ranked brief (company, why WAG, property fit, partnership angle, estimated tier, brand-safety read) for Katie to select from — selected candidates get a real `revenue_companies` row and a `revenue_approvals` row (`action_type=pursue_opportunity`) only once Katie says which ones to actually pursue; researched-but-not-selected candidates are recorded for future reference, not silently discarded, but do not get deal rows created speculatively.

### 38c. Shared functions both roles call (not separate agents)

**Brand-Fit & Conflict Check** — Consolidated §5's existing conflict/exclusivity/brand-safety logic, now with the Pop&Boom case (Section 33b) as its first real, standing example. Runs before either 38a or 38b progresses an opportunity past initial research.

**Pricing Intelligence** — per `WAG_PRICING_INTELLIGENCE_METHODOLOGY.md`: full input set, source-quality-ranked evidence, component breakdown, anti-underpricing protections, calibration discipline. Neither 38a nor 38b commits a number; both request a recommendation from this shared function when an opportunity reaches that point.

**Relationship Brain** — Section 25's existing shared company/people history. Both roles check it before treating any company as new, and both write real evidence back into it (never a private, function-specific record of the same counterparty).

### 38d. What is explicitly not activated yet

Deal Strategy (Section 19), Campaign Architect (Section 30), Offer & Package Intelligence (Section 18) as standalone active functions, Licensing Opportunity Intelligence's proactive research (Section 35) beyond what Epic already evidences, and any collections/AR automation beyond the proven Kidoodle recommendation pattern (Recovery Pass Findings §14a). These stay named and designed, not activated, until 38a/38b prove out over real use — matching Katie's explicit "smallest team" instruction and the standing "prove before expanding" discipline already used for the schema itself.
